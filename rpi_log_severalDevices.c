...
int main(int argc, char **argv)
{
	...
	while (true)
	{
		...
		for (int index_mb = 0, index_s = 0; index_mb < getNbElements(devices_addr); index_mb++)
		{
			...
			{
				//fprintf(stderr, "Error ioctl : (errno %d) %s\r\n", errno, strerror(errno)); //si errno == EINVAL c'est ok
				continue;
			}
			//read a line
			while (true)
			{
				if (read(file_i2c, rx_buff, 2) > 0)
				{
					...
					{
							continue;   //a voir si on break plutôt pour passer à un autre capteur
					}
					...
					if (ch_buffer == '\r' || ch_buffer == '\n')
					{
						...
						//si le devEui courant est vide
						if (emptyDevEui(devInfArr[index_mb][index_s]))
						{
							//alors on vérifie si la chaine courante contient le devEui
							if (isDevEui(str_buffer))
							{
								//si c'est le cas on remplit la structure d'information du capteur
						...
						else
						{
							...
							//on ouvre ensuite le fichier en ajout
